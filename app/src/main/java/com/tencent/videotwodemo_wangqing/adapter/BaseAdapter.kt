...
abstract class BaseAdapter<DATA> : RecyclerView.Adapter<CommonViewHolder> {
    ...
    override fun onBindViewHolder(holder: CommonViewHolder, position: Int) {
        ...
        /*设置条目的长按事件*/
        clickLongValue?.let { holder.itemView.setOnLongClickListener { view -> it(view, adjPosition,mDatas!!.get(adjPosition)); true } }
        /*封装子View的点击事件*/
        chindClickValue?.let {
...
//    ==============================API提供=================================================
     ...
     * 在第一个位置查询一条新数据
     */
    fun setOneData(mData: DATA) {
        if (this.mDatas==null){
            mDatas = mutableListOf<DATA>()
        }
        this.mDatas?.add(0, mData)
        if (mEmptyView != null&&this.mDatas?.size==1){
            notifyDataSetChanged()
        }else{
            notifyItemInserted(mHeads.size())
            notifyItemRangeChanged(mHeads.size(),mDatas?.size?:0+mFoods.size())
        }
     ...
     * 删除指定位置的一个条目
     */
    fun deleteItemData(mData: DATA,position: Int){
        this.mDatas?.remove(mData)
        if (mEmptyView != null&&this.mDatas?.size==0){
            notifyDataSetChanged()
        }else{
            notifyItemRemoved(mHeads.size()+position)
            notifyItemRangeChanged(mHeads.size(),mDatas?.size?:0+mFoods.size())
        }
     ...
     * 刷新指定位置条目
     */
    fun NotifyPositionItem(mData: DATA,position: Int){
        this.mDatas?.set(position,mData)
        notifyItemChanged(mHeads.size()+position)
    }
    ...
    fun  setHeadAndFootLine(isline:Boolean){
        this.isline = isline
    }
